...
function App() {
  ...
  const performWrite = useCallback<PerformWrite>(
    async (functionName, args, expectedState) => {
      if (!wallet.provider || !wallet.address || wallet.status !== "connected") {
      ...
      const client = createWalletClient(wallet.provider, wallet.address);
...
function OverviewScreen({ navigate, refreshToken }: ScreenProps) {
  ...
  useEffect(() => {
    ...
    void loadRegistry().then((result) => {
      if (!mounted) return;
      const partial = result.rows.some((row) => row.error);
      setState({ rows: result.rows, count: result.count, activeCount: partial ? null : result.rows.filter((row) => row.batch?.recallActive === true).length });
      setError(undefined);
      ...
      {error ? <ReadUnavailable message={error} /> : !state ? <LoadingState /> : (
        <>
          <section className="metrics-grid">
            ...
            <Metric label="ACTIVE RECALLS" value={formatCount(state.activeCount)} detail="bounded public scan · actual recall_active" />
...
function AssessmentForm({ batch, wallet, performWrite, onComplete }: { batch: Batch; wallet: WalletState; performWrite: PerformWrite; onComplete: () => void }) {
  ...
  const [error, setError] = useState<string>();
  const submit = async (event: FormEvent) => { event.preventDefault(); setError(undefined); if (!title.trim() || !notes.trim()) { setError("Title and notes are required by the contract."); return; } if (!reviewing) { setReviewing(true); return; } try { const before = batch.assessmentCount; await performWrite("assess_batch", assessmentArgs(batch.batchId, title.trim(), notes.trim(), batch.evidenceSources), async () => { const next = await readBatch(batch.batchId); return next !== null && next.assessmentCount > before; }); onComplete(); } catch (reason) { setError(errorMessage(reason)); } };
...
function RegisterScreen({ performWrite, wallet, navigate }: ScreenProps) {
  ...
  const validate = (): string | null => { const required: Array<[keyof RegisterFormValues, string]> = [["manufacturer", "Manufacturer"], ["productName", "Product name"], ["productModel", "Product model"], ["sku", "SKU"], ["lotNumber", "Lot number"], ["batchCode", "Batch code"], ["recallPolicy", "Recall policy"]]; for (const [key, label] of required) if (!String(values[key]).trim()) return `${label} is required.`; const urls = values.evidenceSources.map((value) => value.trim()).filter(Boolean); if (!urls.length) return "At least one evidence source is required."; if (urls.length > LIMITS.sourceCount) return `No more than ${LIMITS.sourceCount} evidence sources are permitted.`; if (urls.some((value) => !isHttpUrl(value))) return "Every evidence source must be an HTTP(S) URL."; return null; };
  const submit = async (event: FormEvent) => { event.preventDefault(); const validation = validate(); if (validation) { setError(validation); return; } setError(undefined); if (!reviewing) { setReviewing(true); return; } try { const before = await readBatchCount(); await performWrite("register_batch", registerArgs(values), async () => (await readBatchCount()) > before); setReviewing(false); navigate("#/registry"); } catch (reason) { setError(errorMessage(reason)); } };
...
function AssessmentHistoryScreen({ navigate, refreshToken }: ScreenProps) {
  const [state, setState] = useState<{ count: number; assessments: Assessment[]; capped: boolean }>();
  const [error, setError] = useState<string>();
  useEffect(() => { let mounted = true; void (async () => { try { const count = await readAssessmentCount(); const capped = count > LIMITS.assessmentScan; const firstId = Math.max(1, count - LIMITS.assessmentScan + 1); const values = await Promise.all(Array.from({ length: count === 0 ? 0 : count - firstId + 1 }, (_, index) => readAssessment(firstId + index).catch(() => null))); const assessments = values.filter((value): value is Assessment => value !== null).sort((left, right) => right.assessmentId - left.assessmentId); if (mounted) setState({ count, assessments, capped }); } catch (reason) { if (mounted) setError(errorMessage(reason)); } })(); return () => { mounted = false; }; }, [refreshToken]);
  return <div className="screen"><PageHeader eyebrow="03 / ASSESSMENT HISTORY" title="Consensus record"><span className="header-note">GLOBAL ASSESSMENT INDEX<br /><span>REVIEWER PROVENANCE / COMMITMENTS</span></span></PageHeader>{error ? <ReadUnavailable message={error} /> : !state ? <LoadingState /> : <><div className="registry-toolbar"><span>{formatCount(state.count)} total assessments</span><span className="muted">{state.capped ? `Showing newest ${LIMITS.assessmentScan} addressable records.` : "Latest-nonfinal public reads"}</span></div>{state.assessments.length ? <div className="history-list">{state.assessments.map((assessment) => <button className="history-row" key={assessment.assessmentId} onClick={() => navigate(`#/batch/${assessment.batchId}`)}><span className="registry-id">#{String(assessment.assessmentId).padStart(4, "0")}</span><span><strong>{assessment.title}</strong><small>Batch #{assessment.batchId} · sequence {assessment.sequenceNumber}</small></span><StatusChip status={assessment.verdict} tone={assessment.verdict === "AFFECTED" ? "critical" : assessment.verdict === "NOT_AFFECTED" ? "safe" : "watch"} /><span className="muted">{shortHash(assessment.evidenceCommitment)}</span><span>→</span></button>)}</div> : <ReadUnavailable message="NO ASSESSMENTS RECORDED" />}</>}</div>;
}
